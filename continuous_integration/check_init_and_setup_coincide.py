...
"""Check that the distribution and skileu/__init__.py are in sync."""
...
def main() -> int:
    ...
    if setup_py_map["version"] != skileu.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in skileu/__init__.py is: "
            f"{skileu.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != skileu.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in skileu/__init__.py is: "
            f"{skileu.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != skileu.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in skileu/__init__.py is: "
            f"{skileu.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != skileu.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in skileu/__init__.py is: "
            f"{skileu.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != skileu.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in skileu/__init__.py, "
                f"but found: {skileu.__status__}"
            )
            success = False
